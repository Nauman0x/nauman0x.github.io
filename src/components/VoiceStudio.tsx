...
export const VoiceStudio: React.FC = () => {
  ...
  return (
    <section id="voice" className="py-24 bg-[#0A0713] border-y border-[#201833] relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-1/2 right-10 w-96 h-96 rounded-full bg-[#9D72E8]/10 blur-[120px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 space-y-3">
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl cursor-pointer border transition-all duration-200 ${
                    isCurrent
                      ? 'bg-[#150F26] border-[#9D72E8] shadow-[0_0_20px_rgba(157,114,232,0.25)]'
                      : 'bg-[#0F0C1B] border-[#251D38] hover:border-[#4B396B] hover:bg-[#130E22]'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-sm text-white">{sample.title}</span>
                    <span className="text-[11px] font-mono text-[#9D72E8] px-2 py-0.5 rounded bg-[#9D72E8]/15">
                      {sample.duration}
                    </span>
                  </div>
                  <div className="text-xs font-mono text-[#B794F4] mb-2">{sample.agentType}</div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-4 p-3 rounded-xl bg-[#110D1D] border border-[#271E3C] hover:border-[#9D72E8] text-xs font-mono text-[#C4BED9] flex items-center justify-between group transition-all"
            >
          ...
          <div className="lg:col-span-7 rounded-2xl bg-[#0D0918] border border-[#2B2144] p-6 sm:p-8 flex flex-col justify-between shadow-2xl">
            <div>
              {/* Console Top Bar */}
              <div className="flex items-center justify-between pb-4 border-b border-[#211836] mb-6">
                <div className="flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full bg-[#9D72E8] animate-pulse" />
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">{selectedSample.title}</h3>
              <p className="text-xs font-mono text-[#B794F4] mb-6">{selectedSample.scenario}</p>

              {/* Dynamic Waveform Visualizer */}
              <div className="h-28 rounded-xl bg-[#07050D] border border-[#1E162F] p-4 flex items-end justify-between gap-1 mb-6 overflow-hidden">
                {activeBars.map((height, i) => (
                  <div
                    ...
                    className={`flex-1 rounded-full transition-all duration-100 ${
                      isPlaying
                        ? 'bg-gradient-to-t from-[#805AD5] to-[#B794F4]'
                        : 'bg-[#251D38]'
                    }`}
                  />
                ))}
              ...
              <div className="p-4 rounded-xl bg-[#120D21] border border-[#241B3B] mb-6">
                <div className="text-[11px] font-mono text-[#9D72E8] uppercase tracking-wider mb-1 flex items-center gap-1.5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[#1F1732]">
              <div className="flex items-center gap-3">
                <button
                  onClick={togglePlay}
                  className="px-5 py-2.5 rounded-xl bg-[#9D72E8] hover:bg-[#8B5CF6] text-white font-mono text-xs font-bold flex items-center gap-2 shadow-[0_0_20px_rgba(157,114,232,0.4)] transition-all"
                >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-4 py-2 rounded-xl bg-[#171126] hover:bg-[#251C3D] text-white border border-[#31254D] font-mono text-xs flex items-center gap-2 transition-all"
              >
